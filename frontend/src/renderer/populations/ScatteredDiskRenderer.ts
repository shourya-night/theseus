/**
 * THESEUS Scattered Disk Renderer
 * ===============================
 * GPU-instanced visualization of high-eccentricity, high-inclination
 * trans-Neptunian objects in the scattered disk (perihelion near 30 AU,
 * aphelion reaching 150+ AU).
 *
 * Same architecture as AsteroidBeltRenderer / KuiperBeltRenderer: every
 * member has full Keplerian elements prepared once, Kepler-propagated per
 * frame through CoordinateSystem.
 */

import * as THREE from 'three';
import { SCATTERED_DISK_PARAMS } from '../../data/smallBodies';
import {
  PreparedOrbit,
  prepareOrbit,
  orbitPositionInto,
...
export class ScatteredDiskRenderer {
  ...
  constructor(count = SCATTERED_DISK_PARAMS.count, seed = 0xFACE) {
    this.count = count;
    ...
    const material = new THREE.MeshStandardMaterial({
      color: new THREE.Color(...SCATTERED_DISK_PARAMS.color),
      roughness: 0.94,
      metalness: 0.03,
    });

    this.instancedMesh = new THREE.InstancedMesh(geometry, material, this.count);
    this.instancedMesh.name = 'ScatteredDiskMesh';
  ...
  private initPopulation(seed: number): void {
    const params = SCATTERED_DISK_PARAMS;
    ...
    for (let i = 0; i < this.count; i++) {
      // Scattered disk: semi-major axis skewed toward larger values.
      const a_scene = innerScene + Math.pow(rand(), 1.5) * (outerScene - innerScene);
  ...
  update(simTimeSec: number): void {
    ...
    if (this.lastSimTimeSec !== -Infinity && Math.abs(simTimeSec - this.lastSimTimeSec) < 180) {
      return;
    }
    ...
    for (let i = 0; i < this.count; i++) {
      ...
      this.dummy.rotation.set(this.scratch.x * 0.03, this.scratch.y * 0.03, this.scratch.z * 0.03);
